"""Swoosh-style public deposit / payout feed embeds for crypto."""
...
from modules.log_privacy import log_display_name

LOG_COLOR = 0x9B59B6  # purple accent
...
def _payout_footer() -> str:
    ...
    if not vouches.startswith("#"):
        vouches = f"#{vouches}"
    return (
        f"✦ real on-chain payout ✦ · happy? vouch in {vouches} · "
        "show your name with `/setprivacy public`"
    )


class OnChainExplorerView(discord.ui.View):
...
def build_deposit_received_embed(
    *,
    display_name: str,
    amount_usd: float,
    amount_crypto: float,
    ...
    embed.set_footer(text=_deposit_footer())
    return embed


def build_payout_sent_embed(
    *,
    display_name: str,
    amount_usd: float,
    amount_crypto: float,
    ...
    embed.set_footer(text=_payout_footer())
    return embed
...
async def post_deposit_feed_logs(
    ...
    *,
    ...
    name = log_display_name(user_id, member)
    for dep in credited:
        chain = dep.get("chain", "CRYPTO")
        embed = build_deposit_received_embed(
            display_name=name,
        ...
        url = explorer_tx_url(chain, dep.get("tx_id") or "")
        view = OnChainExplorerView(url) if url else None
        try:
...
async def post_payout_feed_log(
    ...
    *,
    ...
    url = explorer_tx_url(chain, tx_id or "")
    view = OnChainExplorerView(url) if url else None
    try:
        await channel.send(embed=embed, view=view)
    except Exception as e:
