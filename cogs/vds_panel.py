"""Owner control bot — /build and /vds_manage (never loaded on licensed customer instances)."""
...
def _admin_key() -> str:
    ...
    if not key:
        raise RuntimeError("LICENSE_ADMIN_KEY missing in .env")
    return key
...
def _server_url() -> str:
    ...
    if not url:
        raise RuntimeError("LICENSE_SERVER_IP or LICENSE_SERVER_URL missing in .env")
    return url
