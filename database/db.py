...
async def _init_tables():
    ...
    await db.executescript("""
        ...
        VALUES
            ...
            ('gates',     'Gates of Olympus', 1, 10, 100000, 0.05, 0.02),
            ...
            ('market_predict', 'Market Predict', 1, 10, 100000, 0.00, 0.02),
            ('horse_race',   'Horse Race',   1, 10, 100000, 0.00, 0.02);
    """)
    await db.execute(
        "INSERT OR IGNORE INTO games (id, name, enabled, min_bet, max_bet, rigged_chance, house_edge) "
    ...
    await db.execute(
        "INSERT OR IGNORE INTO games (id, name, enabled, min_bet, max_bet, rigged_chance, house_edge) "
        "VALUES ('horse_race', 'Horse Race', 1, 10, 100000, 0.0, 0.02)"
    )
    await db.execute(
        "INSERT OR IGNORE INTO games (id, name, enabled, min_bet, max_bet, rigged_chance, house_edge) "
        "VALUES ('gates', 'Gates of Olympus', 1, 10, 100000, 0.05, 0.02)"
    )
